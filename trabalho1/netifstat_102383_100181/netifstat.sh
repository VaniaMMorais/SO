...
if [[ $flagP == 1 && $flagReverse == 1 && $flagSort == 0 ]]; then 
	order="-r"
fi
	...
	if [[ $flagL == 1 ]]; then
		...
		if [[ $flagC == 1 && ! "${rede}" =~ ${c} ]]; then 		# verificar -c
			if [[ $flagP == 1 ]]; then 
				if [[ $flagSort == 1 && $flagReverse == 1 ]]; then
					...
					cat dados.txt | grep -w $c | head -$((${p}+1)) | ${sort} -rn
			...
			if [[ $flagSort == 1 && $flagReverse == 1 ]]; then
				...
				cat dados.txt | head -$((${p}+1)) | ${sort} -rn
			elif [[ $flagSort == 1 ]]; then
		...
		if [[ $flagC == 1 && ! "${rede}" =~ ${c} ]]; then 		# verificar -c
			if [[ $flagP == 1 ]]; then 
				if [[ $flagSort == 1 && $flagReverse == 1 ]]; then
					cat dados.txt | grep -w $c | head -$((${p}+1)) | ${sort} -n
				elif [[ $flagReverse == 1 && $flagSort == 0 ]]; then 
					cat dados.txt | grep -w $c | head -$((${p}+1)) | ${sort} -rn
				elif [[ $flagSort == 1 && $flagReverse == 0 ]]; then
					cat dados.txt | grep -w $c | head -$((${p}+1)) | ${sort} ${order}
				else
					cat dados.txt | grep -w $c | head -$((${p}+1))
				fi
